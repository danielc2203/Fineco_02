    ...
    fecha_incorporacion = $.trim($('#fecha_incorporacion').val());
        $.ajax({
          ...
          success: function(data) {
            tablacrud.ajax.reload(null, false);
           }
        });
...
$(document).on("click", ".btnBorrar", function(){
    ...
    primer_nombre = $(this).closest("td").find('td:eq(1)').text();	// Nombre de Usuario
    opcion = 3; //eliminar
    var respuesta = confirm("¿Está seguro de borrar el registro " +id+ "?" +primer_nombre);
    if (respuesta) {
        $.ajax({
          url: "bd/crud.php",
          type: "POST",
          ...
          success: function() {
              tablacrud.row(fila.parents('tr')).remove().draw();
           }
        });
    }
 });
